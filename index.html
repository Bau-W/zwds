(function(){
  ...
  $('toggleOverlay').addEventListener('change', renderChart);

  let selectedPalaceBranch = null; // 目前點選看三方四正的宮位

  function sanfangSet(branch){
    return [branch, mod12(branch+4), mod12(branch+8), mod12(branch+6)];
  }
  ...
  function renderChart(){
    ...
    center.innerHTML = `
      ...
      ${currentRec.unknownHour ? '<div style="color:var(--danger)">⚠️ 時辰未知，命宮/身宮及部分星曜僅供參考</div>' : ''}
    `;
    grid.appendChild(center);

    const showOverlay = $('toggleOverlay').checked;
    const sanfang = selectedPalaceBranch !== null ? sanfangSet(selectedPalaceBranch) : null;

    for (let branch = 0; branch < 12; branch++){
      const p = chart.palaceByBranch[branch];
      const pos = BRANCH_GRID[branch];
      const div = document.createElement('div');
      let cls = 'palace' + (p.isLife ? ' life':'') + (p.isBody ? ' body':'');
      if (sanfang){
        if (branch === selectedPalaceBranch) cls += ' sf-self';
        else if (sanfang.includes(branch)) cls += ' sf-related';
      }
      div.className = cls;
      ...
      if (showOverlay){
        let dxTag = '', lnTag = '';
        if (ln.curDaxian){
          const dxRel = mod12(branch - ln.curDaxian.branch);
          dxTag = `<span class="overlay-tag dx">大${E.PALACE_ABBR[dxRel]}</span>`;
        }
        const lnRel = mod12(branch - ln.liuNianPalaceBranch);
        lnTag = `<span class="overlay-tag ln">流${E.PALACE_ABBR[lnRel]}</span>`;
        overlayHtml = `<div class="overlay-names">${dxTag}${lnTag}</div>`;
      }
      ...
      div.innerHTML = `
        ...
        ${overlayHtml}
        <div class="palace-name">${p.palaceName}${p.isBody ? '<span class="body-mark">(身)</span>' : ''}</div>
      `;
      div.addEventListener('click', () => {
        selectedPalaceBranch = (selectedPalaceBranch === branch) ? null : branch;
        renderChart();
      });
      grid.appendChild(div);
    }

    updateSanfangHint(chart);
  ...
  function updateSanfangHint(chart){
    const hint = $('sanfangHint');
    if (selectedPalaceBranch === null){
      hint.textContent = '點任一宮位，可看該宮的三方四正';
      return;
    }
    const p0 = chart.palaceByBranch[selectedPalaceBranch];
    const opp = chart.palaceByBranch[mod12(selectedPalaceBranch+6)];
    const t1 = chart.palaceByBranch[mod12(selectedPalaceBranch+4)];
    const t2 = chart.palaceByBranch[mod12(selectedPalaceBranch+8)];
    hint.innerHTML = `<strong>${p0.palaceName}</strong> 三方四正：對宮【${opp.palaceName}】、三合【${t1.palaceName}】【${t2.palaceName}】`;
  }
